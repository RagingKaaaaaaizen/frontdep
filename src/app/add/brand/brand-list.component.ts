...
export class BrandListComponent implements OnInit {
  ...
  applyFilters() {
    ...
    switch (this.selectedSort) {
      case 'name':
        ...
        break;
      case 'items':
        filtered.sort((a, b) => this.getBrandItemCount(b.id) - this.getBrandItemCount(a.id));
        break;
      case 'created':
  ...
  getBrandItemCount(brandId: number): number {
    return this.items.filter(item => item.brandId === brandId).length;
  }
  ...
  getAverageItemsPerBrand(): number {
    if (this.brands.length === 0) return 0;
    return Math.round(this.items.length / this.brands.length);
  }

  getMostPopularBrand(): string {
    if (this.brands.length === 0) return 'N/A';
    
    let maxItems = 0;
    let popularBrand = 'N/A';
    
    this.brands.forEach(brand => {
      const itemCount = this.getBrandItemCount(brand.id);
      if (itemCount > maxItems) {
        maxItems = itemCount;
        popularBrand = brand.name;
      }
    });
  ...
  viewBrand(id: number) {
    this.router.navigate(['/add/brand/view', id]);
  }
    ...
    if (confirm('Are you sure you want to delete this brand?')) {
      this.brandService.delete(id)
        ...
        .subscribe({
          ...
          error: (error) => {
            this.alertService.error('Error deleting brand');
          }
        });
    }
  }
